...
public class DAO <T> {
    ...
    protected List listar(T objeto, String entidad) {

        conectar();
        List<T> obj = em.createQuery("SELECT F FROM " + entidad + " F").getResultList();
   ...
   /* protected <T> T consultar(String consulta, Class<T> tipoResultado) {
        
        conectar();
        
        T obj = em.createQuery(consulta, tipoResultado).getSingleResult();
